...
#Imports list from Ninja. That list is created by exporting all workstations and uploading to the IT Share.
$NinjaList = Import-Csv \\*SERVER*\IT\EOL\NinjaExport.csv
...
foreach ($Computer in $NinjaList){
    ...
    #If the users is not disabled, gets the computer object and adds it to to CSV. Otherwise, moves on to the next computer.
    if ("$User.Enabled" -ne "False") {
        ...
        Export-Csv -Path \\*SERVER*\it\EOL\EOLList_$FileDate.csv -Append
    }
}